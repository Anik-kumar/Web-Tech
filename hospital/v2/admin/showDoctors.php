...
if($_SESSION['usertype'] == "Admin") {
    $id = $_REQUEST['id'];
    $conn = getConnection();
    $query = "SELECT emp.*, dept.dept_name FROM department dept LEFT JOIN employee emp ON dept.dept_id=emp.dep_id WHERE user_type='Doctor'";
...
<div id="upperDiv-showAdmin">
    ...
    <h2 class="blue-backgnd">Doctors </h2>
...
<script type="text/javascript">
    $("#deleteBtn").click(function (e) {
        if(window.confirm("Are You Sure?")){
            e.preventDefault();
            window.location.href("deleteEmpValidation.php?empId=<?= $row['emp_id']; ?>&user=emp&page=showDoctors");
        }
    });
</script>